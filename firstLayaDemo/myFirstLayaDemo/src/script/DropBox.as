package script {
	...
	public class DropBox extends Script {
		...
		override public function onEnable():void {
			...
			var curScore:int = GameUI.instance._score; // 当前分数
			this._rig = this.owner.getComponent(RigidBody);
			// 根据当前分数调整游戏难度
			var levelByScore:Number = Math.ceil(Math.random() * curScore / 5) + 1
			// 单个盒子最大值为9
			this.level = levelByScore < 10 ? levelByScore : 9;
			// this.level = Math.ceil(Math.random() * 5) + 1;
		...
		override public function onTriggerEnter(other:*,self:*,contact:*):void {
			...
			if (other.label === "buttle") {
				...
				// 增加游戏乐趣, 盒子落地不结束游戏, 减少落地盒子的等级
				owner.removeSelf(); // 移除落地盒子
				GameUI.instance.lessScore(this.level);
			}
		}
